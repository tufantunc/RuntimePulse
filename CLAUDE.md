...
Stages 1–3 implemented and merged to main: core engine (store/bus/engine/daemon/client/CLI), watchers (http, tcp, file, process, docker, git + `exec` wrapper), and the continuation dispatcher with the Claude Code adapter (per-session FIFO, supervised resumes, chaining via `continuation.completed/failed` events, boot recovery). Next per spec §13: Cursor/Codex/OpenCode adapters and the MCP server, then workflow YAML/WebSocket.

Commands: `go build ./...`, `go test -race ./...`, `go test -run TestName ./internal/...`, `go vet ./...`, `gofmt -l .` (must print nothing), `golangci-lint run`, end-to-end: `./scripts/smoke.sh` (hermetic, must end `SMOKE OK`; mocks the claude CLI via `RUNTIMEPULSE_CLAUDE_BIN`). Package layout: `internal/core` (domain types), `internal/store` (SQLite, single-connection — NEVER touch `s.db` while a tx is open), `internal/bus`, `internal/engine`, `internal/watch`, `internal/adapter`, `internal/dispatch`, `internal/daemon`, `internal/client`, `cmd/runtimepulse`. Prompts are rendered at ingest and stored on the continuation — the dispatcher reads `c.Prompt`, never re-renders. v1 resumes carry NO permission flags (owner decision); graceful shutdown leaves in-flight continuations `running` for boot recovery.

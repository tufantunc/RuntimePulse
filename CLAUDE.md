...
ALL SIX STAGES of spec §13 implemented and merged to main: core engine (store/bus/engine/daemon/client/CLI), watchers (http, tcp, file, process, docker, git + `exec` wrapper), the continuation dispatcher (per-session FIFO, supervised resumes, chaining via `continuation.completed/failed` events, boot recovery), the MCP server (`internal/mcpserver`, official `modelcontextprotocol/go-sdk` v1.6.1, stdio: `create_watch`/`create_rule`/`wait_for_event`/`get_events`/`cancel_rule`), ALL FOUR agent adapters (claude/cursor/codex/opencode on the shared supervised `runCLI` runner in `internal/adapter/run.go`; binaries overridable via `RUNTIMEPULSE_<AGENT>_BIN`; prompts positional after `--`), the workflow YAML compiler (`internal/workflow` + `runtimepulse apply`, compiles to oneShot rules — no workflow entity in core), and the opt-in WebSocket event stream (`internal/wsserver`, `daemon --ws-port`, 127.0.0.1-only + token in `~/.runtimepulse/ws-token`). Claude Channels optimization is DEFERRED while the feature is a research preview. Stage 7 added WINDOWS SUPPORT (Win10 1803+): AF_UNIX unchanged on all platforms; build-tagged shims (`lock_*.go`, `sock_*.go`, `detach_*.go`); process watching via gopsutil/v4 on ALL platforms (owner-approved exception to the no-SDK rule — pattern = case-sensitive substring of full cmdline); test mocks use the `internal/mockexe` re-exec pattern (NEVER write `#!/bin/sh` mocks in Go tests); 3-OS GitHub Actions matrix in `.github/workflows/ci.yml` (gofmt+smoke unix-only). Windows gate for every change: `GOOS=windows go build ./... && GOOS=windows go vet ./...`. Actual Windows test execution happens only in CI (requires push). DISTRIBUTION SHIPPED: v1.0.0 (tag-driven GoReleaser → 6 archives + SHA256SUMS + Homebrew CASK on tufantunc/homebrew-tap + checksum-verifying scripts/install.sh; release flow in docs/RELEASING.md; version = git tag via ldflags into `internal/version`). Stage 9 added the MCP SETUP WIZARD: `runtimepulse setup` (`internal/setup`) detects installed agent CLIs and registers the MCP server — native `claude/codex mcp add` (runCmd seam, already-exists tolerated), atomic JSON merge for cursor (`~/.cursor/mcp.json`) and opencode (`~/.config/opencode/opencode.json`); codex is global-only (TOML never hand-edited).

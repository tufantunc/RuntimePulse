...
Stages 1–5 implemented and merged to main: core engine (store/bus/engine/daemon/client/CLI), watchers (http, tcp, file, process, docker, git + `exec` wrapper), the continuation dispatcher (per-session FIFO, supervised resumes, chaining via `continuation.completed/failed` events, boot recovery), the MCP server (`internal/mcpserver`, official `modelcontextprotocol/go-sdk` v1.6.1, stdio: `create_watch`/`create_rule`/`wait_for_event`/`get_events`/`cancel_rule`), and ALL FOUR agent adapters (claude/cursor/codex/opencode on a shared supervised `runCLI` runner in `internal/adapter/run.go`; binaries overridable via `RUNTIMEPULSE_<AGENT>_BIN`; prompts positional after `--`). Next per spec §13: workflow YAML compiler, WebSocket streaming.
